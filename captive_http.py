...
class HTTPServer(Server):
    ...
    def __init__(self, poller, local_ip):
        ...
        self.request = dict()
        self.conns = dict()
        self.routes = {b"/": b"./index_ap.html",
                       b"/login": self.login,
                       b"/ssid_list": self.get_ssid_list,
                       b"/wifi_status": self.get_wifi_status}
    ...
    def get_ssid_list(self, params):
        ...
        for ssid in self.ssid_list:
            # ssid = ssid.encode('raw_unicode_escape').decode('utf-8')
            json = json + ssid[0].decode('utf-8') + '", "'
        json = json + '"]'
        print(json)
        return json, headers
    ...
    def login(self, params):
        ...
        # self.wifi_status = b"connecting"
        return self.wifi_status, headers

    def get_wifi_status(self, params):
        headers = b"HTTP/1.1 200 OK\r\n"
        return self.wifi_status, headers

    def connected(self, params):
        headers = b"HTTP/1.1 200 OK\r\n"
        body = open("../BoatMCU/connected.html", "rb").read() % (self.ssid, self.local_ip)
        return body, headers
    ...
    def is_valid_req_data(self, data):

        if not (data[0] == ord('G') and data[1] == ord('E') and data[2] == ord('T') and data[3] == ord(' ')):
            return False
        match = False
    ...
    def read(self, s):
        ...
        # if not self.is_valid_req_data(data):
        #     self.close(s)
        #     return
        ...
        print("sid", sid)
        if method == b'POST':
            self.request[sid] = b'POST '
            print("found post request", sid)
        else:
            method = self.request.get(sid, b'')
            if method == b'POST ':
                print('Found post ign data', sid)
            if self.request.get(sid, b'') != b'POST ':
